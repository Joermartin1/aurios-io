(function () {
  const terms = [
    ...
    { term: 'RAG',                 def: 'Retrieval-Augmented Generation — a technique where relevant documents are fetched and injected into an AI prompt before generating a response.' },
    ...
    { term: 'embedding',           def: 'Converting text into a vector (a list of numbers) that captures its meaning, enabling semantic search and similarity matching.' },
    { term: 'embeddings',          def: 'Numerical representations of text meaning. Enable semantic search — finding content by meaning rather than exact words.' },
  ];
    ...
    for (const { term, def } of terms) {
      ...
      if (regex.test(html)) {
        html = html.replace(regex, match => {
          ...
          return `<span class="auri-tip" data-tip="${safedef}">${match}</span>`;
        });
        modified = true;
      }
    }
